 ...
 * @return errno_t
 */
errno_t leafnode_attachleaf(
    CLUSTERTREE *ctree,
    long CFindexleaf,
    long CFindexleafnode
)
{
    DEBUG_TRACE_FSTART();
    ...
    while(cfi!= -1)
    {
        ...
        if(cfi == cfip)
        {
            FUNC_RETURN_FAILURE("Attaching leaf %ld to node %ld: CF parent %ld points to itself",
                                CFindexleaf, CFindexleafnode, cfi);
        }
