 ...
 * @return errno_t
 */
errno_t leafnode_attachleaf(CLUSTERTREE *ctree, long CFindexleaf, long CFindexleafnode)
{
    DEBUG_TRACE_FSTART();

    ctree->CFarray[CFindexleafnode].leafindex[ctree->CFarray[CFindexleafnode].NBleaf] = CFindexleaf;
    ctree->CFarray[CFindexleafnode].NBleaf++;

    ctree->CFarray[CFindexleaf].parentindex = CFindexleafnode;
    ctree->CFarray[CFindexleaf].level = ctree->CFarray[CFindexleafnode].level + 1;
    ...
    while (cfi != -1)
    {
        ...
        addvector_to_CF(ctree, ctree->CFarray[CFindexleaf].datasumvec, ctree->CFarray[CFindexleaf].datassq,
                        ctree->CFarray[CFindexleaf].N, cfi, &addOK);
        ...
        if (cfi == cfip)
        {
            FUNC_RETURN_FAILURE("Attaching leaf %ld to node %ld: CF parent %ld points to itself", CFindexleaf,
                                CFindexleafnode, cfi);
        }
