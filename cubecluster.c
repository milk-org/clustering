    ...
    for(long cfi=0; cfi<ctree->NBCF; cfi++)
    {
        if(ctree->CFarray[cfi].type != CLUSTER_CF_TYPE_UNUSED)
        {
            ...
            if(ctree->CFarray[cfi].type == CLUSTER_CF_TYPE_NODE)
            {
                ...
                {
                    FUNC_RETURN_FAILURE(
                        "node %ld at level %d number of childred %d exceeds limit %d",
                        cfi,
                        ctree->CFarray[cfi].level,
            ...
            if(ctree->CFarray[cfi].type == CLUSTER_CF_TYPE_LEAFNODE)
            {
                ...
                {
                    FUNC_RETURN_FAILURE(
                        "node %ld at level %d number of leaves %d exceeds limit %d",
                        cfi,
                        ctree->CFarray[cfi].level,
    ...
    while(ctree->CFarray[CFindex].NBchild > 0)
    {
        ...
        for(long childi=0; childi<ctree->CFarray[CFindex].NBchild; childi++)
        {
            ...
            DEBUG_TRACEPOINT("computing distance to CF node # %ld(%ld)",
                             CFindex1, ctree->CFarray[CFindex1].N);
        ...
        DEBUG_TRACEPOINT("level %3d %4ld  %g",
...
)
{
    ...
    for(long leafi=0; leafi<ctree->CFarray[CFindex].NBleaf; leafi++)
    {
        ...
        DEBUG_TRACEPOINT("computing distance to leaf %ld = node # %ld(%ld)",
                         leafi, CFindex1, ctree->CFarray[CFindex1].N);
    ...
    for(long frame=0; frame < NBframe; frame++)
    {
        ...
        if(frameskip == 0)
        {
            ...
            else
            {
                long CFindex;
                FUNC_CHECK_RETURN(
                    findleafnode(&ctree, datarray, &CFindex)
                );
                ...
                long leafi;
                FUNC_CHECK_RETURN(
                    findleaf(&ctree, datarray, CFindex, &leafi)
                );
                ...
                if(leafi != -1)
                {
                    ...
                    int addOK = 0;
                    FUNC_CHECK_RETURN(
                        leaf_addentry(&ctree, datarray, ssqr, lCFindex, &addOK)
                    );
                    ...
                    {
                        DEBUG_TRACEPOINT("Radius condition not met for leaf index %ld",
                                         leafi);
                ...
                if(leafi == -1)
                {
                    ...
                    DEBUG_TRACEPOINT("CREATED LEAF at index %ld",
                                     nCFindex);
                    ...
                    if(ctree.CFarray[CFindex].NBleaf == ctree.L+1)
                    {
                        ...
                        {
                            DEBUG_TRACEPOINT("SPLITTING NODE %ld",
                                             upCF);
